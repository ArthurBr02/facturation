// Conforming quote HTML, rendered server-side from a frozen snapshot.
// Same visual language as factureHtml.js — only the meta block changes.
...
/** Render the full HTML document for a devis snapshot. */
export function renderDevisHtml(snap) {
  ...
  const acompteBlock =
    d.acomptePct
      ? `<div class="line"><span>Acompte à la commande (${Number(d.acomptePct)} %)</span><span class="amt">${euro((snap.totalHt * Number(d.acomptePct)) / 100)}</span></div>`
      : '';
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .clauses{margin-top:30px;display:flex;flex-direction:column;gap:12px}
  .clause-block{border-left:3px solid var(--line);padding:8px 14px;font-size:12.5px;color:var(--muted)}
  ...
  .cycles-badge{display:inline-flex;align-items:center;gap:6px;background:#f3f5fb;border:1px solid #dfe3fa;border-radius:20px;padding:4px 12px;font-size:12px;color:var(--accent);margin-top:6px}
  ...
  .validity-note{margin-top:10px;font-size:12px;color:var(--faint);text-align:right}
</style></head>
<body>
  ...
  <div class="head">
    ...
    <div class="invoice-meta">
      ...
      <div class="meta-grid">
        <div class="k">N°</div><div class="v">${esc(d.numero)}</div>
        <div class="k">Date d'émission</div><div class="v">${frDate(d.dateEmission)}</div>
        <div class="k">Validité jusqu'au</div><div class="v">${frDate(d.dateValidite)}</div>
        ${d.bonCommande ? `<div class="k">Réf. commande</div><div class="v">${esc(d.bonCommande)}</div>` : ''}
      </div>
    </div>
  </div>
  ...
  <div class="parties">
    <div class="card client">
      <div class="eyebrow">Devis établi pour</div>
    ...
    <div class="card">
      ...
      ${d.titre ? `<div class="cline" style="font-weight:600;color:var(--ink)">${esc(d.titre)}</div>` : ''}
      ${d.description ? `<div class="cline" style="margin-top:6px;white-space:pre-wrap">${esc(d.description)}</div>` : ''}
      <div class="cycles-badge">
        ...
        ${d.cyclesInclus || 3} révision(s) mineure(s) incluse(s)
      </div>
    </div>
  </div>
  ...
  <div class="totals">
    <div class="totals-box">
      <div class="line"><span>Total HT</span><span class="amt">${euro(snap.totalHt)}</span></div>
      <div class="tva-note">TVA non applicable — franchise en base (HT = TTC)</div>
      ${acompteBlock}
      <div class="net"><span class="lbl">Total devis</span><span class="amt">${euro(snap.totalHt)}</span></div>
    </div>
  </div>

  ${d.clauseRevision || d.clauseHebergement ? `
  <div class="clauses">
    ${d.clauseRevision ? `
    <div class="clause-block">
      <div class="clause-title">Révisions</div>
      <div>${esc(d.clauseRevision)}</div>
    </div>` : ''}
    ${d.clauseHebergement ? `
    <div class="clause-block">
      <div class="clause-title">Hébergement</div>
      <div>${esc(d.clauseHebergement)}</div>
    </div>` : ''}
  </div>` : ''}

  <div class="validity-note">
    Devis valable jusqu'au ${frDate(d.dateValidite)}. Passée cette date, le devis est caduc.
  </div>
...
export default { renderDevisHtml };
