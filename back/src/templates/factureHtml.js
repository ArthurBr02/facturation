// Conforming invoice/credit-note HTML, rendered server-side from a frozen
// snapshot (see facture.service buildSnapshot). Derived from models/facture.html
// but static: no toolbar, no contenteditable, no scripts. HT = TTC (293 B CGI).
//
// The single source for the PDF look lives here, so the future Factur-X switch
// only touches pdf.service + this template.
...
// Document title shown in the meta block, by facture type.
const TYPE_LABEL = {
  standard: 'Facture',
...
/** Render the full HTML document for a facture snapshot. */
export function renderFactureHtml(snap) {
  const em = snap.emetteur || {};
  const cl = snap.client || {};
  const f = snap.facture || {};
  const mentions = snap.mentions || {};
  const isAvoir = f.type === 'avoir';
  const label = TYPE_LABEL[f.type] || 'Facture';
...
<body>
  ...
  <div class="head">
    ...
    <div class="invoice-meta">
      ...
      <div class="meta-grid">
        <div class="k">N°</div><div class="v">${esc(f.numero)}</div>
        <div class="k">Date d'émission</div><div class="v">${frDate(f.dateEmission)}</div>
        ${isAvoir ? '' : `<div class="k">Échéance</div><div class="v">${frDate(f.dateEcheance)}</div>`}
        ${f.bonCommande ? `<div class="k">Bon de commande</div><div class="v">${esc(f.bonCommande)}</div>` : ''}
        ${f.factureOrigineNumero ? `<div class="k">Facture d'origine</div><div class="v">${esc(f.factureOrigineNumero)}</div>` : ''}
      </div>
    </div>
  </div>
  ...
  <div class="parties">
    <div class="card client">
      <div class="eyebrow">${isAvoir ? 'Avoir au profit de' : 'Facturé à'}</div>
      ...
      ${cl.tvaIntra ? `<div class="row"><span class="k">TVA intra.</span><span class="v">${esc(cl.tvaIntra)}</span></div>` : ''}
    </div>
    <div class="card">
      <div class="eyebrow">Prestation</div>
      ${f.objet ? `<div class="cline" style="font-weight:600;color:var(--ink)">${esc(f.objet)}</div>` : '<div class="cline">Prestation de services</div>'}
      ${f.dateExecutionDebut && f.dateExecutionFin
        ? `<div class="cline" style="margin-top:8px">Période d'exécution : <b>${frDate(f.dateExecutionDebut)}</b> → <b>${frDate(f.dateExecutionFin)}</b></div>`
        : f.dateExecutionDebut
          ? `<div class="cline" style="margin-top:8px">Date d'exécution : <b>${frDate(f.dateExecutionDebut)}</b></div>`
          : ''}
    </div>
  </div>
  ...
  <div class="totals">
    <div class="totals-box">
      <div class="line"><span>${isAvoir ? "Total de l'avoir" : 'Total HT'}</span><span class="amt">${euro(snap.totalHt)}</span></div>
      <div class="tva-note">TVA non applicable — franchise en base (HT = TTC)</div>
      <div class="net"><span class="lbl">${isAvoir ? 'Montant avoir' : 'Net à payer'}</span><span class="amt">${euro(snap.totalHt)}</span></div>
    </div>
  </div>
  ...
  <div class="pay">
    <div class="col">
      ...
      <div class="v">${esc(f.conditionsReglement || 'Par virement, à réception de facture.')}</div>
      ${em.iban ? `<div class="mono" style="margin-top:8px"><b>IBAN :</b> ${esc(em.iban)}</div>` : ''}
      ${em.bic ? `<div class="mono"><b>BIC :</b> ${esc(em.bic)}</div>` : ''}
    </div>
  </div>

  <div class="legal">
    <span class="tva-flag">${esc(mentions.tva || 'TVA non applicable, art. 293 B du CGI')}</span><br>
    ${esc(mentions.penalites || '')}
  </div>
</body></html>`;
}

export default { renderFactureHtml };
